...
Game::Game() : m_window("The Adventure in Caves", sf::Vector2u(W_WIDTH, W_HEIGHT)),
               m_player("levels/level1.txt"),
               m_hud(&m_player)
{
    loadLevels();
    m_restarted = false;
...
void Game::update()
{
    ...
    for (Enemy* enemy : m_world[m_which_world].m_weak_enemies)
        if (enemy->getCollider().checkCollision(m_player.getCollider(), m_player.m_collision_direction, 0.0f))
        {
            ...
            // If player is in range of enemy for more than 1.2 sec, then enemy can attack.
    ...
    // If enemy has 0 (or less) health points, erase it from vector and add points for player
    ...
    for (auto it = m_world[m_which_world].m_strong_enemies.begin(); it != m_world[m_which_world].m_strong_enemies.end();)
        if ((*it)->getHealth() <= 0)
        {
