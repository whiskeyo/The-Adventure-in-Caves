...
Enemy::Enemy(int l_type, sf::Vector2f l_position, int l_damage, int l_health)
    : m_action(Action::NONE), m_frame(0), m_damage(l_damage), m_health(l_health)
{
    ...
    {
    case 6:
        m_enemy_sprite.setTexture(ResourceManager::getInstance()->requestTexture("enemy_weak_left"), true);
        m_bounding_box_offset = -15;
        break;
    case 7:
        m_enemy_sprite.setTexture(ResourceManager::getInstance()->requestTexture("enemy_weak_right"), true);
        m_bounding_box_offset = 15;
        break;
    case 8:
        m_enemy_sprite.setTexture(ResourceManager::getInstance()->requestTexture("enemy_strong_left"), true);
        m_bounding_box_offset = -15;
        break;
    case 9:
        m_enemy_sprite.setTexture(ResourceManager::getInstance()->requestTexture("enemy_strong_right"), true);
        m_bounding_box_offset = 15;
        break;
    default:
        break;
...
void Enemy::animation(int l_frames, int l_ms, int l_starting_x, int l_starting_y)
{
    if (m_anim_clock.getElapsedTime() > sf::milliseconds(l_ms))
    {
        ...
        m_enemy_sprite.setTextureRect(
            sf::IntRect(sf::Vector2i(l_starting_x + 64 * m_frame, l_starting_y), E_FRAMESIZE));
    }
}
...
void Enemy::update()
{
    ...
    // m_enemy_sprite.setTextureRect(sf::IntRect(sf::Vector2i(512, 0),
    // E_FRAMESIZE));
    ...
    m_bounding_box.setPosition(m_enemy_sprite.getPosition().x + m_bounding_box_offset, m_enemy_sprite.getPosition().y);
}
