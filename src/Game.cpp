...
Game::Game()
    : m_window("The Adventure in Caves", sf::Vector2u(W_WIDTH, W_HEIGHT)),
      m_player("assets/levels/level1.txt"), m_hud(&m_player) {
  loadLevels();
  m_restarted = false;
  m_which_world = 0;
  m_clock.restart();
...
Game::~Game() { ResourceManager::getInstance()->cleanUp(); }
...
void Game::loadLevels() {
  ...
  m_world.push_back(World("assets/levels/level4.txt"));
  m_world.push_back(World("assets/levels/level5.txt"));
  m_world.push_back(World("assets/levels/goodjob.txt"));
}
...
void Game::update() {
  ...
  for (Block &block : m_world[m_which_world].m_blocks)
    if (block.getCollider().checkCollision(
            m_player.getCollider(), m_player.m_collision_direction, 1.0f))
  ...
  for (Enemy *enemy : m_world[m_which_world].m_weak_enemies)
    if (enemy->getCollider().checkCollision(
            m_player.getCollider(), m_player.m_collision_direction, 0.0f)) {
  ...
  // for player
  for (auto it = m_world[m_which_world].m_weak_enemies.begin();
       it != m_world[m_which_world].m_weak_enemies.end();)
    if ((*it)->getHealth() <= 0) {
  ...
  for (auto it = m_world[m_which_world].m_coins.begin();
       it != m_world[m_which_world].m_coins.end();)
    if ((*it)->getCollider().checkCollision(
            m_player.getCollider(), m_player.m_collision_direction, 0.0f)) {
  ...
  for (auto it = m_world[m_which_world].m_hearts.begin();
       it != m_world[m_which_world].m_hearts.end();)
    if ((*it)->getCollider().checkCollision(
            m_player.getCollider(), m_player.m_collision_direction, 0.0f)) {
  ...
  for (Portal *portal : m_world[m_which_world].m_portals)
    if (portal->getCollider().checkCollision(
            m_player.getCollider(), m_player.m_collision_direction, 0.0f)) {
      if (portal->whichType() == 1 &&
          portal->m_portal_clock.getElapsedTime() > sf::seconds(1.0f))
